...
1. Fork 本仓库并按上面的步骤部署到你自己的 Cloudflare Pages。
2. 打开你部署好的域名下的 `/admin`，首次进入会要求设置后台密码、GitHub Token，以及 **GitHub 仓库**（格式：`你的用户名/你的仓库名`）。
3. 仓库信息保存后，后台的所有内容操作（发文章、改配置、管理友链）都会作用于你自己的仓库，不会影响原仓库。
4. 如果之后需要更换仓库或更新 Token，登录时在对应输入框填入新值即可覆盖旧值，留空则继续使用已保存的值。
5. 登录后进入「站点设置」页，顶部会看到一张红色的「初始化为新站点」卡片，勾选要清空的内容（文章/友链），在确认框输入 `INIT` 后点击「初始化为我的博客」，即可一次性清空原作者的示例文章和友链。该操作**仅能执行一次**，执行后卡片会自动隐藏，避免误触二次清空。
