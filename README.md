...
## 两个分支：main（空白模板）与 zsfan（我自己在用）

本仓库有两个独立、互不影响的分支，各自都是完整可运行的项目：

| 分支 | 用途 | 内容 |
|---|---|---|
| `main` | 空白模板，专门给 Fork 本项目的人使用 | `posts/`、`friends/` 为空，`config/site.config.ts` 是占位配置 |
| `zsfan` | 我自己的博客，Cloudflare Pages 生产环境部署的就是这个分支 | 我写的所有文章、友链、真实站点配置 |

两个分支各自独立、自包含，谁部署谁的内容，互不覆盖：我在 `zsfan` 分支上发文章、改配置，只会提交到 `zsfan`；我以后修 bug、加新功能则提交到 `main`，同步给 Fork 的人。Fork 的人默认拿到的是 `main` 分支（空白模板），不会带着我的文章。

## Fork 后如何配置为自己的博客

1. Fork 本仓库，默认拉取的是 `main` 分支（空白模板，没有示例文章/友链），按上面的步骤部署到你自己的 Cloudflare Pages。
2. 打开你部署好的域名下的 `/admin`，首次进入会要求设置后台密码、GitHub Token，以及 **GitHub 仓库**（格式：`你的用户名/你的仓库名`）。
3. 仓库信息保存后，后台的所有内容操作（发文章、改配置、管理友链）都会作用于你自己的仓库 `main` 分支，不会影响原仓库。
4. 如果之后需要更换仓库或更新 Token，登录时在对应输入框填入新值即可覆盖旧值，留空则继续使用已保存的值。
5. 由于 `main` 分支本身就是空白模板，通常不需要额外初始化。如果你想重新清空一次（比如测试后想重来），登录后进入「站点设置」页，顶部会看到一张红色的「初始化为新站点」卡片，勾选要清空的内容（文章/友链），在确认框输入 `INIT` 后点击「初始化为我的博客」即可。该操作**仅能执行一次**，执行后卡片会自动隐藏。
