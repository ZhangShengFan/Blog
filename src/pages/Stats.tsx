...
export const Stats = () => {
  ...
  return (
    <div className="pb-10 md:pb-20">
      <Seo title="统计" description="ZSFan 的博客 站点统计概览：文章数、总字数、分类标签、图片数量等核心数据一目了然。" />
      ...
      <section className="mt-8 grid gap-5 md:mt-10 lg:grid-cols-3">
        <div className="rounded-2xl border border-zinc-200/80 bg-white/90 p-5 dark:border-zinc-800/80 dark:bg-zinc-900/90 lg:col-span-1">
          <h3 className="mb-4 font-serif text-lg font-bold text-zinc-900 dark:text-zinc-100">最近更新</h3>
          <div className="space-y-3">
            {(siteStats.recentPosts || []).map((post) => (
              <a key={post.id} href={`/post/${post.id}`} className="block rounded-xl border border-zinc-100 p-3 transition-colors hover:border-zinc-300 hover:bg-zinc-50 dark:border-zinc-800 dark:hover:border-zinc-700 dark:hover:bg-zinc-800/50">
